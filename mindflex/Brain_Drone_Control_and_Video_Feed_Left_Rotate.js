...
var serialPort_1 = new com.SerialPort(port1, {
    baudrate: 9600,
    parser: com.parsers.readline('\r\n')
});
var serialPort_2 = new com.SerialPort(port2, {
    baudrate: 9600,
    parser: com.parsers.readline('\r\n')
});
var serialPort_3 = new com.SerialPort(port3, {
    baudrate: 9600,
    parser: com.parsers.readline('\r\n')
});
serialPort_1.on('open', function() {
    console.log('ONE: ' + port1 + ': Port open');
});
serialPort_2.on('open', function() {
    console.log('TWO: ' + port2 + ': Port open');
});
serialPort_3.on('open', function() {
    console.log('TRE: ' + port3 + ': Port open');
});
...
function getReading() {
    if (dontGetReading == 0) {
        ...
        if (testConnection_1 == 200) { //if headset is not on, set attention1 to 0
            ...
            //attention1 = array1[1];  //set Attention to the
            //attention value from Processing(no longer usin processing - serial going
            //straight into node)
        ...
        /*while launched, if Headset 2's attention>benchmark, rotate Left,
		else, stop rotating*/
